...
module.exports.listen = function(io, socket) {
	//create a game
	socket.on("createGame", ({ userToken, lang, player2 }) => {
		console.log("5. in bacckend createGame");
	...
	socket.on("playerInGame", (player) => {
		console.log("2. player In game Running");
	...
	socket.on("gameRequest", ({ token, gameId, invitedPlayer }) => {
		const game = games.find((g) => {
			return g.gameId.toString() === gameId.toString();
		});
		if (!game) {
	...
	socket.on("gameOver", (gameId, outcome, currentPlayer) => {
		...
		if (!game) {
			...
			removeGameFromUser(game.player1.playerId);
			removeGameFromUser(game.player2.playerId);
