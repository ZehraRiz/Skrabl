...
const GameScreen = ({ setNotification, chat, handleSendMessage, setCurrentComponent, gameData, iam }) => {
	const [ selectedTile, setSelectedTile ] = useState(null);
	const [ selectedSquareIndex, setSelectedSquareIndex ] = useState(null);
	const [ playerRackTiles, setPlayerRackTiles ] = useState(
		iam === 0 ? gameData.gameState.player1Tiles : gameData.gameState.player2Tiles
	);
	...
	const [ timeLeftPlayer, setTimeLeftPlayer ] = useState(
		iam === 0 ? gameData.gameState.player1TimeLeft : gameData.gameState.player2TimeLeft
	);
	const [ timeLeftOpponent, setTimeLeftOpponent ] = useState(
		iam === 0 ? gameData.gameState.player1TimeLeft : gameData.gameState.player2TimeLeft
	);
	const [ currentPlayer, setCurrentPlayer ] = useState(gameData.gameState.turn);
	...
	//DUMMY DATA //will be set as state
		...
		<div className="gameScreen__wrapper">
			<div className="gameScreen__main">
				<div className="gameScreen__board">
					...
					/>
					<TileRack playerRackTiles={playerRackTiles} handleSelectTile={handleSelectTile} />
					<GameButtons
						getTiles={getTiles}
			...
			{gameIsOver && <GameOverModal winner="Tom" scores={scores} handleCloseModal={handleCloseModal} />}
			{confirmMessage && (
				<ConfirmModal
