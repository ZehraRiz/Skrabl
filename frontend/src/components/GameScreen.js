...
const GameScreen = ({
			...
			.then((res) => {
				setTimeout(() => {
					if (res.data.exchange && pouch.length > 0) {
						...
						setComputerConsecutivePasses(0);
						const newWords = findWordsOnBoard(res.data.newBoardState, res.data.tilesUsed).filter(
							(word) => word.newWord === true
						);
						const [ turnPoints, turnHighScore ] = getTurnPoints(newWords, res.data.tilesUsed, turn);
	...
	useEffect(
		() => {
			if (consecutivePasses > 5 || (consecutivePasses > 1 && pouch.length === 0)) {
	...
	const handleClickPlacedTile = (tileToRemove) => {
		if (
			selectedTile === 0 ||
			currentPlayer !== turn ||
			placedTiles.filter((tile) => tile.square == tileToRemove.square).length === 0
		)
			return;
	...
	const handleConfirmExchange = () => {
		...
		const updatedRack = playerRackTiles.filter((tile) => !idsToRemove.includes(tile.id));
	...
	const handleClickConfirmMove = () => {
		...
		if (moveIsValid(placedTiles, boardState)) {
			...
			axios
				...
				.then((res) => {
					...
					if (Object.values(results).every((val) => val === "true")) {
						const [ turnPoints, turnHighScore ] = getTurnPoints(newWords, placedTiles, turn);
						const playerPreviousPoints = scores[turn];
						const updatedScores = {
							...scores,
							[turn]: playerPreviousPoints + turnPoints
						...
						setNotification(
							`${notifications["The following word was not found in the dictionary:"][
								lang
							]} ${invalidWords[0]}`
						);
					}
			...
			if (scores[0] === 0 && scores[1] === 0 && placedTilesIndices.indexOf(112) === -1) {
				setNotification(notifications["The first word on the board must use the centre square."][lang]);
				return;
			} else {
